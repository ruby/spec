describe :net_ftp_getbinaryfile, :shared => :true do
  ...
  it "returns nil" do
    @ftp.send(@method, "test", @fixture_file).should be_nil
  end
  ...
  describe "when passed a block" do
    it "yields the received content as binary blocks of the passed size" do
      res = []
      @ftp.send(@method, "test", @fixture_file, 10) { |bin| res << bin }
  ...
  describe "when resuming an existing file" do
    before(:each) do
      @fixture_file = File.dirname(__FILE__) + "/../fixtures/getbinaryfile_resume"
    ...
    describe "and the REST command fails" do
      it "raises a Net::FTPProtoError when the response code is 550" do
        ...
        lambda { @ftp.send(@method, "test", @fixture_file) }.should raise_error(Net::FTPProtoError)
      end

      it "raises a Net::FTPPermError when the response code is 500" do
        @server.should_receive(:rest).and_respond("500 Syntax error, command unrecognized.")
        lambda { @ftp.send(@method, "test", @fixture_file) }.should raise_error(Net::FTPPermError)
      end
      ...
      it "raises a Net::FTPTempError when the response code is 421" do
        ...
        lambda { @ftp.send(@method, "test", @fixture_file) }.should raise_error(Net::FTPTempError)
      end
