...
describe "Hash.[]" do
  ...
  it "coerces a single argument which responds to #to_ary" do
    ...
    hash_class[ary].should == { a: :b }
  end

  it "ignores elements that are not arrays" do
    hash_class[[:a]].should == {}
    hash_class[[:nil]].should == {}
  end

  it "raises an ArgumentError for arrays of more than 2 elements" do
    lambda{ hash_class[[[:a, :b, :c]]].should == {} }.should raise_error(ArgumentError)
  end
  ...
  it "raises an ArgumentError when passed an odd number of arguments" do
    ...
    lambda { hash_class[1, 2, { 3 => 4 }] }.should raise_error(ArgumentError)
  end

  it "calls to_hash" do
    obj = mock('x')
    def obj.to_hash() { 1 => 2, 3 => 4 } end
    hash_class[obj].should == { 1 => 2, 3 => 4 }
  end
