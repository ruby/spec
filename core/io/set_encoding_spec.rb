  ...
  describe "IO#set_encoding" do
    ...
    ruby_bug "http://redmine.ruby-lang.org/issues/5567", "1.9.3" do
      it "does not parse encoding names separated by ':' when passed a second argument" do
        lambda { @io.set_encoding("utf-8:utf-16be", "us-ascii") }.should complain(/Unsupported encoding utf-8:utf-16be ignored/)
      end
    end

    it "warns if passed the name of a nonexistent Encoding" do
      lambda { @io.set_encoding("nonexistent") }.should complain(/Unsupported encoding nonexistent ignored/)
    end
